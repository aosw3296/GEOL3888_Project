...
chemdata = numpy.loadtxt("/Users/Emily/Downloads/EarthChemSN.txt", delimiter=',')
...
#South America 26°08′54.29 South, longitude 60°53′57.85″
where_longitude_above_min = chemdata[:,1]<-10
sndata=chemdata[where_longitude_above_min]
#Repeat for less than 180
where_longitude_below_max = sndata[:,1]>-100
sndata2=sndata[where_longitude_below_max]

#Repeat for latitude
sndata3=sndata2[sndata2[:,0]<10]
sndata4=sndata3[sndata3[:,0]>-50]

#Assigning the latitude, longitude, age and abundance according to South America
latlon=sndata4[:,0:2]
lat = sndata4[:,0]
lon = sndata4[:,1]
...
#In[16]
#Plate ID
plateIDs = numpy.full(len(lon),201)
print plateIDs
print latlon
print lonlat
...
tic=time.time()
b=coregLoop(sndata4[:,0:2],sndata4[:,2], plateIDs)
toc=time.time()
print("Time taken:", toc-tic, " seconds")
...
tic=time.time()
c=coregLoop(sndata4[:,0:2], randomAges, sndata4[:,3])
toc=time.time()
print("Time taken:", toc-tic, " seconds")
